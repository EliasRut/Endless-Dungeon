...
export type MyNote = 'A' | 'A#' | 'B' | 'C' | 'C#' | 'D' | 'D#' | 'E' | 'F' | 'F#' | 'G' | 'G#';
...
(() => {
	function add(note: MyNote, n: number) {
...
export function textNoteToNumber(note: FullNote) {
	...
	const n: MyNote = note.substring(0, note.length - 1) as MyNote;
...
export function midiNoteToText(note: number): FullNote {
	...
	const noteName = revLook.get(n) as MyNote;
...
export function Audio(
	au: AudioContext = new (window.AudioContext || (window as any).webkitAudioContext)()
) {
