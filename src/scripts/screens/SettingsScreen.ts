...
export default class SettingsScreen extends OverlayScreen {
	...
	constructor(scene: Phaser.Scene) {
		...
		fileInput.addEventListener(
			'change',
			() => {
				this.load(this.loadJSONFile(fileInput));
			},
			false
		);
	...
	async load(savegame: any): Promise<any> {
		...
		if (savegame.playerCharacter)
			localStorage.setItem('playerCharacter', JSON.stringify(savegame.playerCharacter));
		...
		if (savegame.transitionStack)
			localStorage.setItem('transitionStack', JSON.stringify(savegame.transitionStack));
		if (savegame.availableRooms)
			localStorage.setItem('availableRooms', JSON.stringify(savegame.availableRooms));
		if (savegame.availableTilesets)
			localStorage.setItem('availableTilesets', JSON.stringify(savegame.availableTilesets));
		if (savegame.currentLevel)
			localStorage.setItem('currentLevel', JSON.stringify(savegame.currentLevel));
		if (savegame.roomAssignment)
			localStorage.setItem('roomAssignment', JSON.stringify(savegame.roomAssignment));
		if (savegame.inventory) localStorage.setItem('inventory', JSON.stringify(savegame.inventory));
		if (savegame.saveGameName)
			localStorage.setItem('saveGameName', JSON.stringify(savegame.saveGameName));
